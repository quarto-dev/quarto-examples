...
In some specific case where you don't get the output you expect from specific feature on tables (for example, when producing tables with a R function), you may require to opt-out Quarto Tables processing by adding a specific attributes on the table ([Docs](https://quarto.org/docs/authoring/tables.html#disabling-quarto-table-processing))
...
Quarto has specific features for tables : https://quarto.org/docs/authoring/tables.html. It was designed to handle Tables natively for a good output in all formats. For that Quarto will do a specific processing for Markdown tables, but also for HTML tables that will be parsed the same way as Markdown tables. 

If this has a lot of advantage for offering some specific feature to tables, it will also have the side effect on tables produced by other tools. That is why Quarto offers an opt-out mechanism to [disable Quarto processing](https://quarto.org/docs/authoring/tables.html#disabling-quarto-table-processing) using a specific attributes on tables
...
Tables can also be produced using R packages as R has several packages able to produce HTML tables. These packages often introduced some specific feature for tables, which Quarto could ignore and even overwrite.
